  ...
  const handleCategory = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    switch (e.target.value) {
      case 'all':
        ...
        break;
      case 'entertainment':
        let entertainment = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(entertainment)
        break;
      case 'bills':
        let bills = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(bills)
        break;
      case 'groceries':
        let groceries = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(groceries)
        break;
      case 'dining out':
        let diningOut = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(diningOut)
        break;
      case 'transportation':
        let transportation = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(transportation)
        break;
      case 'education':
        let education = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(education)
        break;
      case 'personal care':
        let personalCare = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(personalCare)
        break;
      case 'lifestyle':
        let lifestyle = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(lifestyle)
        break;
      case 'shopping':
        let shopping = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
        setFilteredTransactions(shopping)
        break;
      case 'general':
        let general = transactions.filter(transaction => transaction.category.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()))
  ...
  const reduceSort = () => {
    switch (sortOption) {
      ...
      case 'lowest':
        ...
        break;
      default:
        let def = searchTerm ? filteredTransactions.filter((transaction) => transaction.name.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())) : filteredTransactions
        setFilteredTransactions(def)
        break;
    }
  }
    ...
    <section className='bg-white rounded-xl p-400'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-250'>
          <div>
            <label htmlFor='sort-select' className='text-4 text-grey-500 mr-100'>Sort by</label>
          ...
          <div>
            <label htmlFor='cat-select' className='text-4 text-grey-500 mr-100'>Category</label>
